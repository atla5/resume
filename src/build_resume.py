...
def update_resume_values(dict_values):
    ...
    filtered = [ed for ed in educations if "craft" in ed['tags'] or "software" in ed['tags']]
    ...
    work_xp = [xp for xp in experiences if "craft" in xp['tags']]
    for i, work_experience in enumerate(work_xp[:2], start=1):
        generate_work_experience(dict_values, work_experience, pre="WA", id=i)

    # work experiences B (e.g. "software")
    pro_xp = [xp for xp in experiences if "software" in xp['tags']]
