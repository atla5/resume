...
# license: MIT
# purpose: generate a customized LaTeX document from template, json, user input
...
def generate_school_info(school, id=None):
...
def generate_work_experience(work, id=1):
    prefix="W{}_".format(id)
    responsibilities = work.responsibilities if work.responsibilities else []
...
def generate_language_entry(level, languages, id=1):
    suffix="_{}".format(id)
    dict_values.update({
...
def generate_languages(languages):
    ...
    generate_language_entry(LVL_1, ls_intermediate, 1)
    generate_language_entry(LVL_2, ls_functional, 2)
    generate_language_entry(LVL_3, ls_limited, 3)
